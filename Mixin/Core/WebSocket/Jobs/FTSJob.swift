...
class FTSJob: BaseJob {
    ...
    override func run() throws {
        ...
        repeat {
            ...
            let stmt = try MixinDatabase.shared.database.prepareUpdateSQL(sql: "INSERT INTO fts_messages(docid, message_id, conversation_id, content, name) SELECT rowid, id, conversation_id, content, name FROM messages WHERE rowid >= ? AND category in ('SIGNAL_TEXT', 'SIGNAL_DATA','PLAIN_TEXT','PLAIN_DATA') AND status != 'FAILED' ORDER BY rowid ASC LIMIT 1000")
